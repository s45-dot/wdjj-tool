...
def test_output_size(small_rgb_image):
    ...
    result = generate_nine_patch_image(small_rgb_image, {}, {})
    assert result.size == (src_w + 2, src_h + 2)
...
def test_four_corners_transparent(small_rgb_image):
    """Four corners of border are transparent (0,0,0,0)."""
    result = generate_nine_patch_image(small_rgb_image, {}, {})
    w, h = result.size
    corners = [(0, 0), (w - 1, 0), (0, h - 1), (w - 1, h - 1)]
    for x, y in corners:
        assert result.getpixel((x, y)) == (0, 0, 0, 0), f"Corner ({x},{y}) not transparent"
